...
describe("client auth", () => {
  ...
  describe("login", () => {
    ...
    it("should return errorLoginNotAdmin when user is not an admin", async () => {
      const { signInWithEmailAndPassword } = await import("firebase/auth");
      const { getDoc, doc } = await import("firebase/firestore");
      const { login } = await import("./auth");
      ...
      if (E.isLeft(result)) {
        expect(result.left).toBe("errorLoginNotAdmin");
      }
    });

    it("should return errorInvalidAdmin when admin is not valid", async () => {
      const { signInWithEmailAndPassword } = await import("firebase/auth");
      const { getDoc, doc } = await import("firebase/firestore");
      const { login } = await import("./auth");
      ...
      expect(E.isLeft(result)).toBe(true);
      if (E.isLeft(result)) {
        expect(result.left).toBe("errorInvalidAdmin");
      }
    });
